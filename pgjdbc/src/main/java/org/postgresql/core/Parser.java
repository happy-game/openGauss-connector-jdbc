...
public class Parser {
              ...
              if (inParen == 0) {
                  ...
                  if (nativeSql.length() > 0) {
                      ...
                      if (splitStatements) {
                          ...
                          if (!isValuesFound
                                  || !isCurrentReWriteCompatible
                                  || valuesBraceClosePosition == -1
                                  || (bindPositions != null
                                          && valuesBraceClosePosition
                                                  < bindPositions.get(bindPositions.size() - 1))) {
  ...
  public static  String removeFirstComment(String str){
      ...
      str = str.trim();
      String[] queryArr = queryTemp.split("[\n\r]");
      if(queryArr[0].trim().startsWith("--")){
      	if(queryArr.length > 1){
      		int index = str.indexOf(queryArr[1],queryArr[0].length());
      ...
      }else if(queryTemp.startsWith("/*") && queryTemp.contains("*/")){
      	int index = str.indexOf("*/");
      	str = str.substring(index+2);
      }else if(queryTemp.startsWith(";")){
      	str = str.substring(1);
      }
      if(startWithComment(str)){
      	str = removeFirstComment(str);
      }
   ...
   * PostgreSQL 9.4 allows column names like , so
